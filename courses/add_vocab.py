...
def add_new_vocab(cur):
    
    # get the relevant data
    
    # add to vocab item (check if there). if already there with definition: FLAG
    
    # add to relevant course (check if there)
    ...
    def get_vocab_id(word, pos, definition):
        
        COMMAND = """SELECT id, definition FROM vocab
        ...
        if r:
            if len(r) == 1 and ((not r[0][1]) or (r[0][1]==definition)):
                UPD_COMMAND = """UPDATE vocab
                SET definition=%s
                WHERE id=%s
                """
                cur.execute(UPD_COMMAND, (definition, r[0][0]))
                return r[0][0]
            else:
                print("ALREADY DEFINED VOCAB EXISTING WITH CONFLICTING DEFINITION")
                print(r)
                return None
        ...
        if not r:
            ...
            print(word)
            print(rank)
            INS_COMMAND = """INSERT INTO vocab(word, pos, rank, definition)
            VALUES(%s, %s, %s, %s)
            RETURNING id"""
            
            cur.execute(INS_COMMAND, (word, pos, rank, definition))
            l = cur.fetchall()[0][0]
    ...
    COMMAND = """INSERT INTO course_vocab(course_id, vocab_id, counts)
    VALUES(%s, %s, 0)
    """
    ...
    for item in lines:
        ...
        vocab_id = get_vocab_id(word, pos, definition)
        ...
        if not cur.fetchall():
            ...
            cur.execute(COMMAND, (COURSE_NO, vocab_id))
